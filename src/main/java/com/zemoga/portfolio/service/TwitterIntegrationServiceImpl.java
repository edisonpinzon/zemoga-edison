...
public class TwitterIntegrationServiceImpl implements TwitterIntegrationService {
	...
	public TwitterIntegrationServiceImpl(@Value("${integration.twitter.bearer-token}") String bearerToken,
			...
			@Value("${integration.twitter.api-url}") String apiUrl, @Autowired RestTemplate restTemplate) {
	...
	public List<String> getLastTweetsFromUser(String user) {
		...
		boolean isValidUser = !StringUtils.isEmpty(user) && isValidTwitterUsername(user);
		if (isValidUser) {
	...
	/**
	 * @author edisonpinzon 
	 * Method getLastTweetsFromUser 
	 * Param user
	 * Call  GET Request Twitter
	 */
	...
	private List<String> retrieveLastTweets(String user) {
		...
		try {
			...
			ResponseEntity<String> response = restTemplate.exchange(twitterEndpoint, HttpMethod.GET, entity,
					String.class);
			if (response.getStatusCode() == HttpStatus.OK) {
				tweets = parseTwitterResponseToOnlyTweets(response.getBody());
			} else {
				logger.error(
						"Incorrect status received from twitter integration %s, message: %s" + response.getStatusCode(),
						response.getBody());
			}
		} catch (Exception e) {
	...
	/**
	 * @author edisonpinzon 
	 * Method parseTwitterResponseToOnlyTweets 
	 * Param String body
	 * This method Parse Json response
	 */
